(function(){
	...
	var Popup = function(){
		...
		const URL_RATE = "https://chrome.google.com/webstore/detail/flash-video-downloader/aiimdkdngfcipjohbjenkahhlhccpdbc/reviews";
...
console.log(media);		
		...
		this.init = function(){		
			...
			// подгоним размеры			
			setTimeout( function() {	
				var box1 = document.getElementById("captureDownloadHint").getBoundingClientRect();
				if (box1.height == 0) {
					document.getElementById("download_item_container").setAttribute("style", "height: 456px");
				}
			}, 100);	
		}
...
//console.log(media);									
										media.sort( function( item1, item2 )  {	  
														return (item1.priority < item2.priority ? 
																	1 : (item1.priority == item2.priority ? 
																			(item1.id < item2.id ? 1 : -1) : -1));  
													} );
